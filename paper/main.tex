...
\caption{Pilot results regenerated from raw JSONL. The reference policy checks benchmark mechanics. DeepSeek rows are a 72-row live API run. The Kimi row is a one-case live smoke at the official \texttt{.cn} endpoint, not a full provider benchmark.}
\label{tab:pilot}
\end{table}

The deterministic reference policy is not a model benchmark. It verifies that the case definitions, simulator, metrics, and aggregation logic are internally consistent. The live DeepSeek run contains 72 model-backed rows. One row is classified as a parse error. Parser failures are classified separately from provider availability failures. The Kimi row is a one-case endpoint-validation smoke at \texttt{https://api.moonshot.cn/v1/chat/completions}; it executed successfully but is not used for provider-level conclusions.

In this pilot, the unstructured no-harness prompt failed all cases under the action-plan scoring because it usually did not emit executable tool actions. The text-only thin-contract prompt achieved a higher success rate than the thick checklist variant, but both still failed many stateful cases. Appendix Tables~\ref{tab:category-summary} and~\ref{tab:failure-taxonomy} show that the remaining failures are dominated by routing and state-diff errors, not permission violations. This supports the strong-contract argument in a narrow sense, while also sharpening it. A contract written only as natural language is still a prompt. A production harness needs enforceable schemas, tool APIs, parsers, retries, and state checks around that contract.
