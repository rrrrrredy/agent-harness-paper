...
The benchmark is small, synthetic, and designed for clarity rather than statistical generality. The deterministic reference runner checks harness mechanics, not model capability. Live API results are subject to model-version, endpoint, authentication, parameter, latency, and sampling changes. Provider differences in the tables should therefore be read as artifact-specific observations about these runs, not as general performance claims. The case studies are engineering evidence, not controlled deployments across independent organizations. These limitations make the claims bounded: the paper proposes a design framework and pilot evaluation shape, not a universal benchmark.
