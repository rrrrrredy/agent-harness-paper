...
The security, memory, and self-audit repositories are used as artifact examples for three recurring harness concerns: preflight scanning, bounded memory, and periodic health checks. The inventory associates skill-security-guard with deterministic risk dimensions, persistent-memory with local layered memory, and agent-self-audit with checks over skills, memory, cron setup, configuration, and workspace state. These examples support the taxonomy but should not be read as an independent source-code audit.
...
The raw rows are the source of reported pilot metrics. GitHub metadata snapshots support artifact and PR identification, not implementation-behavior claims beyond the bounded case-study interpretation.
