...
The reviewable empirical basis for this paper has three artifact layers. First, a generated GitHub snapshot records metadata for first-party skill, memory, security, self-audit, and AI-radar repositories, plus upstream OpenClaw and Hermes pull requests used as boundary examples. Second, a source evidence audit records path-level and keyword-signal evidence for first-party repositories without storing source bodies. Third, the pilot benchmark contributes raw JSONL runs, regenerated tables, failure inspection, and prompt-hash metadata for live model calls. Formative notes informed the vocabulary around routing contracts, skill interfaces, memory boundaries, and thin-versus-thick harnesses, but artifact claims rely on the committed evidence layers. This evidence structure is intentionally conservative: repository metadata supports artifact existence and scope; source-signal snapshots support bounded artifact interpretation; only raw run files support reported pilot metrics.
...
\caption{Pilot results regenerated from raw JSONL. The reference policy checks benchmark mechanics. DeepSeek and Kimi each contribute 72 live API rows. These pilot rows are not a general provider benchmark.}
\label{tab:pilot}
\end{table}

The deterministic reference policy is not a model benchmark. It verifies that the case definitions, simulator, metrics, and aggregation logic are internally consistent. The live DeepSeek and Kimi runs each contain 72 model-backed rows. DeepSeek has one parse-error row; Kimi has two parse-error rows. Parser failures are classified separately from provider availability failures.

In this pilot, the unstructured no-harness prompt usually failed under action-plan scoring because it did not emit executable tool actions or violated tool boundaries. Thin-contract prompting outperformed the thick checklist in both live runs: DeepSeek improved from 0.458 to 0.542 task success, and Kimi improved from 0.750 to 0.875. These differences are descriptive pilot observations, not provider rankings: the runs use one small synthetic case set, one prompt family, one endpoint configuration per provider, and no repeated trials. Appendix Tables~\ref{tab:live-uncertainty} and~\ref{tab:live-paired-delta} report Wilson intervals and paired bootstrap deltas to make that uncertainty explicit. Both models still failed some stateful and replay cases. Appendix Tables~\ref{tab:category-summary} and~\ref{tab:failure-taxonomy} show that the remaining failures are dominated by routing, state-diff, and parser errors rather than provider availability. This supports the strong-contract argument in a narrow sense, while also sharpening it. A contract written only as natural language is still a prompt. A production harness needs enforceable schemas, tool APIs, parsers, retries, and state checks around that contract.

All live provider runners read credentials only from process environment variables~\cite{deepseekapi,kimiapi}. The Kimi runner uses the \texttt{max\_completion\_tokens} request field and the \texttt{api.moonshot.cn} service address documented by the Kimi Open Platform.
...
This suggests a hypothesis for future work: teams with direct access to model training signals, product traces, and failure analysis may identify useful harness hooks earlier. The relevant hooks are places where an agent loses state, confuses instructions with data, overuses tools, ignores uncertainty, or fails to recover. Under this hypothesis, harnesses become part of the feedback loop: trajectory traces, state diffs, safety failures, and review labels can inform evaluation, post-training, and product design.
...
The benchmark is small, synthetic, and designed for clarity rather than statistical generality. The deterministic reference runner checks harness mechanics, not model capability. Live API results are subject to model-version, endpoint, authentication, parameter, latency, and sampling changes. Wilson intervals and paired bootstrap deltas are included only as descriptive uncertainty checks over the committed pilot rows; they are not inferential claims about provider populations. Provider differences in the tables should therefore be read as artifact-specific observations about these runs, not as general performance claims. The case studies are engineering evidence, not controlled deployments across independent organizations. These limitations make the claims bounded: the paper proposes a design framework and pilot evaluation shape, not a universal benchmark.
...
\begin{itemize}[leftmargin=*]
  ...
  \item regenerated aggregate and uncertainty tables in \texttt{results/tables/};
...
\begin{table}[h]
\centering
\input{../results/tables/live_success_uncertainty.tex}
\caption{Live-run success and state-diff rates with Wilson 95\% intervals. These intervals describe the committed pilot rows only.}
\label{tab:live-uncertainty}
\end{table}

\begin{table}[h]
\centering
\input{../results/tables/live_pairwise_delta.tex}
\caption{Paired pilot deltas for thin-contract versus thick-checklist prompting. Delta is task-success(thin) minus task-success(thick); intervals are deterministic paired bootstrap intervals over cases.}
\label{tab:live-paired-delta}
\end{table}

\begin{table}[h]
\centering
\input{../results/tables/category_summary.tex}
\caption{Pilot results by provider, prompt variant, and case category.}
\label{tab:category-summary}
\end{table}

\begin{table}[h]
\centering
\input{../results/tables/failure_taxonomy.tex}
