...
- Parse-error rerun: 3 DeepSeek rows were rerun after the provider wrapper was fixed to preserve raw model text on parse failures. Rerun rows carry `rerun_reason=parse_error`. Current aggregate tables count 2 parse-error rows: one rerun row that still returned malformed JSON and one additional legacy parser failure that was not rerun in the current raw file.
